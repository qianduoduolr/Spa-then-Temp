...
from mmcv.runner.hooks.evaluation import *
from torch.utils.data import DataLoader
from .eval_utils import copy_params
...
class EvalIterHook(Hook):
    ...
    def __init__(self, dataloader, interval=1, **eval_kwargs):
        ...
        self.output_dir = self.eval_kwargs.pop('output_dir', None)
    ...
    def after_train_iter(self, runner):
        ...
        from vcl.apis import single_gpu_test
        ...
        self.evaluate(runner, results, output_dir=self.output_dir)

    def evaluate(self, runner, results, output_dir):
        """Evaluation function.
        ...
        eval_res = self.dataloader.dataset.evaluate(
            results, output_dir=output_dir, logger=runner.logger)
...
class DistEvalIterHook(EvalIterHook):
    ...
    def after_train_iter(self, runner):
        ...
        if not self.every_n_iters(runner, self.interval):
            return
        runner.log_buffer.clear()
        from vcl.apis import multi_gpu_test
        ...
        if runner.rank == 0:
            print('\n')
            self.evaluate(runner, results, self.output_dir)


@HOOKS.register_module()
class EvalHook_Custom(Hook):
...
@HOOKS.register_module()
class DistEvalHook_Custom(EvalHook):
    """Distributed evaluation hook.
    ...
    def __init__(self,
        ...
        if test_fn is None:
            from vcl.apis.test import multi_gpu_test
